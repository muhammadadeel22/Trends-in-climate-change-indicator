...
x_data = data_forest_transpose.index # the  row index is used as the values for the x-axis
y_data = [data_forest_transpose['Germany'], 
          data_forest_transpose['United States'], 
          data_forest_transpose['Nigeria'],
          data_forest_transpose['China'], 
          data_forest_transpose['Brazil'], 
          data_forest_transpose['Australia']]
xlabel = 'Year'
ylabel = 'square Kilometers (km2)'
labels = ['UK', 'Nigeria', 'China', 'Brazil','Germany', 'USA', 'Australia']
colors = ['magenta', 'blue', 'yellow', 'green', 'red', 'purple', 'black']
title = 'Forest Area % in square kilometers'
...
y_data = [data_electricity_transpose['Germany'], 
          ...
          data_electricity_transpose['United Kingdom'],
...
# Here I create a dataframe of Germany which takes some indicators as parameters
data_China = {'Urban pop. growth': data_urban_transpose['China'],
        ...
        'Forest Area': data_forest_transpose['China'],
...
df_China = pd.DataFrame(data_China) # the desired columns are created using the pandas dataframe function
print(df_China)
...
# the indicator of interest is forest area against the other factors from the China dataframe  
...
corr = corr_China
title = 'China'
correlation_heatmap(data, corr, title)
